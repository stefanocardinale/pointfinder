...
def run_sample(args: object):
    ...
    """
    sample: list[dict] = datahandling.get_samples(sample_ids=[args.sample_id]) #this syntax is type annotation. That is sample (the annotation target) is of type list[dict]. this is done before variable assignment and can be useful for use with type checkers
    component: list[dict] = datahandling.get_components(component_names=[COMPONENT['name']], component_versions=[COMPONENT['version']])
    if len(component) == 0:
